...
public class HoneycombBrood extends DirectionalBlock {
	...
	public ActionResultType onBlockActivated(BlockState thisBlockState, World world, BlockPos position, PlayerEntity playerEntity, Hand playerHand, BlockRayTraceResult raytraceResult) {
		...
		if (itemstack.getItem() == Items.GLASS_BOTTLE) {
			...
			world.setBlockState(position, BzBlocks.DEAD_HONEYCOMB_BROOD.get().getDefaultState().with(BlockStateProperties.FACING, thisBlockState.get(BlockStateProperties.FACING)), 3); 
			...
			if (!playerEntity.isCreative()) {
				...
				if (itemstack.isEmpty()) {
					// places honey bottle in hand
					playerEntity.setHeldItem(playerHand, new ItemStack(Items.HONEY_BOTTLE)); 
				} 
				// places honey bottle in inventory
				else if (!playerEntity.inventory.addItemStackToInventory(new ItemStack(Items.HONEY_BOTTLE))) 
				{
					// drops honey bottle if inventory is full
					playerEntity.dropItem(new ItemStack(Items.HONEY_BOTTLE), false);
				}
			}

			if ((playerEntity.dimension == BzDimensionRegistration.bumblezone() || 
					Bumblezone.BzConfig.allowWrathOfTheHiveOutsideBumblezone.get()) && 
				...
				playerEntity.addPotionEffect(new EffectInstance(
							...
							Bumblezone.BzConfig.howLongWrathOfTheHiveLasts.get(),
							2, 
							false,
							Bumblezone.BzConfig.showWrathOfTheHiveParticles.get(), 
							true));
			}
		...
		// Player is feeding larva
			...
			if (!playerEntity.isCreative()) {
				...
				if (itemstack.isEmpty()) {
					...
					playerEntity.setHeldItem(playerHand, new ItemStack(Items.GLASS_BOTTLE)); 
				} 
			...
			if (ModChecking.buzzierBeesPresent && Bumblezone.BzConfig.allowHoneyWandCompat.get()) {
				...
				if (action == ActionResultType.SUCCESS) {
					...
					if ((playerEntity.dimension == BzDimensionRegistration.bumblezone() || 
						...
						// Now all bees nearby in Bumblezone will get VERY angry!!!
						playerEntity.addPotionEffect(new EffectInstance(BzEffects.WRATH_OF_THE_HIVE,
								Bumblezone.BzConfig.howLongWrathOfTheHiveLasts.get(), 2, false,
								Bumblezone.BzConfig.showWrathOfTheHiveParticles.get(), true));
					}
				...
				// Player is feeding larva
				action = BuzzierBeesRedirection.honeyWandGivingHoney(itemstack, thisBlockState, world, position,
						playerEntity, playerHand);
				if (action == ActionResultType.SUCCESS) {
					world.playSound(
							playerEntity, 
							playerEntity.getPosX(),
				...
				if (itemstack.getItem().getRegistryName().toString().equals("buzzierbees:sticky_honey_wand")) {
					if (!world.isRemote) {
						// spawn bee if at final stage and front isn't blocked off
						int stage = thisBlockState.get(STAGE);
						if (stage == 3) {
	...
	/**
	 * Called periodically clientside on blocks near the player to show honey
	 * particles. 50% of attempting to spawn a particle. Also will buzz too based on
	 * stage
	 */
	@Override
	@OnlyIn(Dist.CLIENT)
	...
	/**
	 * Starts checking if the block can take the particle and if so and it passes
	 * another rng to reduce spawnrate, it then takes the block's dimensions and
	 * passes into methods to spawn the actual particle
	 * 
	 */
	@OnlyIn(Dist.CLIENT)
	...
	/**
	 * intermediary method to apply the blockshape and ranges that the particle can
	 * spawn in for the next addHoneyParticle method
	 */
	@OnlyIn(Dist.CLIENT)
