...
public class HoneycombBrood extends DirectionalBlock
{
	...
	if (itemstack.getItem() == Items.GLASS_BOTTLE) {
	    world.setBlockState(position, BzBlocks.DEAD_HONEYCOMB_BROOD.get().getDefaultState().with(BlockStateProperties.FACING, thisBlockState.get(BlockStateProperties.FACING)), 3); // removed honey from this block
	 ...
	 */
		...
		if (itemstack.getItem() == BzItems.SUGAR_WATER_BOTTLE.get()) {
		    if (world.rand.nextFloat() < 0.30F) successfulGrowth = true;
		}
		else {
	...
	if (!world.isAreaLoaded(position, 1)) return; // Forge: prevent loading unloaded chunks when checking neighbor's light
    ...
    /**
     * Called periodically clientside on blocks near the player to show honey particles. 50% of attempting to spawn a particle. Also will buzz too based on stage
     */
    @Override
    @OnlyIn(Dist.CLIENT)
	...
	if (world.rand.nextInt(40) == 0) world.playSound(position.getX() + 0.5D, position.getY() + 0.5D, position.getZ() + 0.5D, SoundEvents.ENTITY_BEE_LOOP, SoundCategory.BLOCKS, soundVolume, 1.0F, true);
    }


    /**
     * Starts checking if the block can take the particle and if so and it passes another rng to reduce spawnrate, it then takes the block's dimensions and passes into methods to spawn the actual particle
     * 
     */
    @OnlyIn(Dist.CLIENT)
