...
public final class HivePillarBiome extends BzBaseBiome {
    public HivePillarBiome() {
        super((new Settings()).surfaceBuilder(new ConfiguredSurfaceBuilder<>(HONEY_SURFACE_BUILDER, HONEY_CONFIG)).precipitation(Biome.Precipitation.NONE).category(Biome.Category.JUNGLE).depth(5F).scale(0.0F).temperature(1.85F).downfall(0.5F).effects((new BiomeEffects.Builder()).waterColor(14402413).waterFogColor(11700268).fogColor(12638463).method_24943(SoundEvents.BLOCK_BEEHIVE_WORK).build()).parent((String) null));
        ...
        this.addSpawn(EntityCategory.MONSTER, new Biome.SpawnEntry(EntityType.CAVE_SPIDER, 2, 1, 1));
