...
public class HoneycombHoleDecorator extends Decorator<NopeDecoratorConfig>
{
	...
	public HoneycombHoleDecorator(Function<Dynamic<?>, ? extends NopeDecoratorConfig> configFactory)
	{
		super(configFactory);
	}
	...
	public Stream<BlockPos> getPositions(IWorld world, ChunkGenerator<? extends ChunkGeneratorConfig> chunkGenerator, Random random, NopeDecoratorConfig placementConfig, BlockPos pos)
	{
		...
		for(int repeat = 0; repeat < 2; repeat++)
		{
			...
			{
				//setOffsets back and forth in z coordinate so the holes alternate to make this layer of holes
				if(alternate) 
				{
	...
	private boolean isSliceValid(IWorld world, BlockPos pos) 
	{
		...
		for(double z = -4.5; z <= 4.5; z++)
		{
			for(double y = -3.5; y <= 3.5; y++)
			{
				...
				{
					if(!world.getBlockState(pos.add(0, y+1, z)).isOpaque())
					{
						return false;
					}
