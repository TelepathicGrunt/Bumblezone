...
public class HoneyBottleDispenseBehavior extends DefaultDispenseItemBehavior
{
	...
	public ItemStack dispenseStack(IBlockSource source, ItemStack stack)
	{
		...
		if (blockstate.getBlock() == BzBlocks.HONEYCOMB_BROOD.get())
		{
			...
			int stage = blockstate.get(HoneycombBroodBlock.STAGE);
			if (stage == 3)
			{
				//the front of the block
				BlockPos.Mutable blockpos = new BlockPos.Mutable(position);
				blockpos.move(blockstate.get(HoneycombBroodBlock.FACING).getOpposite());
				...
				if (!world.getBlockState(blockpos).getMaterial().isSolid())
				{
					...
					world.setBlockState(position, blockstate.with(HoneycombBroodBlock.STAGE, Integer.valueOf(0)));
				}
			}
			else
			{
				world.setBlockState(position, blockstate.with(HoneycombBroodBlock.STAGE, Integer.valueOf(stage + 1)));
			}
