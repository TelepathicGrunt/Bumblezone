...
public class FakeIncenseCandleRecipeCreator {
    ...
    public static List<CraftingRecipe> constructFakeRecipes(IncenseCandleRecipe incenseCandleRecipe, boolean oneRecipeOnly) {
        ...
        for (Potion potion : potions) {
            if (potion.getEffects().stream().allMatch(e -> Registry.MOB_EFFECT.getHolderOrThrow(Registry.MOB_EFFECT.getResourceKey(e.getEffect()).orElseThrow()).is(BzTags.DISALLOWED_INCENSE_CANDLE_EFFECTS))) {
                continue;
            }
