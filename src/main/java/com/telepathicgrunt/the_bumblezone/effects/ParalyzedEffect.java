...
public class ParalyzedEffect extends MobEffect {
    ...
    public void addAttributeModifiers(LivingEntity entity, AttributeMap attributes, int amplifier) {
        ...
        MobEffectInstance effect = entity.getEffect(BzEffects.PARALYZED);
        if (effect == null) {
            return;
        }
        ...
        if (effect.getDuration() > BzConfig.paralyzedMaxTickDuration) {
            ((MobEffectInstanceAccessor)effect).setDuration(BzConfig.paralyzedMaxTickDuration);
        }

        if(!entity.isRemoved() && entity.level instanceof ServerLevel) {
            MobEffectClientSyncPacket.sendMobEffectSyncPacket(entity, effect);
        }
    }
    ...
    public void removeAttributeModifiers(LivingEntity entity, AttributeMap attributes, int amplifier) {
        ...
        if(!entity.isRemoved() && effect != null && entity.level instanceof ServerLevel) {
            MobEffectClientSyncPacket.sendMobEffectSyncPacket(
                entity,
    ...
    public static boolean isParalyzed(LivingEntity livingEntity) {
        ...
        return livingEntity.hasEffect(BzEffects.PARALYZED);
    }
    ...
    public static boolean isParalyzedClient(LivingEntity livingEntity) {
        ...
        MobEffectInstance effect = livingEntity.getEffect(BzEffects.PARALYZED);
        if(effect != null) {
            if(effect.getDuration() <= 0) {
                livingEntity.removeEffect(BzEffects.PARALYZED);
            }
