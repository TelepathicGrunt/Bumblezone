...
public class WrathOfTheHiveEffect extends MobEffect {
    ...
    public static void calmTheBees(Level world, LivingEntity livingEntity) {
        SEE_THROUGH_WALLS.range(BzConfig.aggressionTriggerRadius);
        List<Bee> beeList = world.getNearbyEntities(Bee.class, SEE_THROUGH_WALLS, livingEntity, livingEntity.getBoundingBox().inflate(BzConfig.aggressionTriggerRadius));
        for (Bee bee : beeList) {
            if (bee.isNoAi()) {
                continue;
            }
