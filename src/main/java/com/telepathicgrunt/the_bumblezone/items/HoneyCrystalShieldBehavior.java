        ...
        if ((source.isExplosion() || source.isFire())) {
            if (player.getUseItem().getItem() instanceof HoneyCrystalShield) {
                ...
                if (source.isExplosion() && player.isBlocking()) {
                    ...
                    ((PlayerDamageShieldInvoker) player).callDamagedShield(Math.max(player.getUseItem().getMaxDamage() / 3, 18));
                }
                else if (source.isFire()) {
                    if(source.isProjectile()) {
                        ((PlayerDamageShieldInvoker) player).callDamagedShield(Math.max(player.getUseItem().getMaxDamage() / 6, 3));
                    }
                    else{
                        ((PlayerDamageShieldInvoker) player).callDamagedShield(Math.max(player.getUseItem().getMaxDamage() / 100, 3));
