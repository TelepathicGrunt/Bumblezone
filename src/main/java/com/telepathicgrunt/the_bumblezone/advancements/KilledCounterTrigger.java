...
public class KilledCounterTrigger extends SimpleCriterionTrigger<KilledCounterTrigger.Instance> {
    ...
    public void trigger(ServerPlayer serverPlayer, Entity currentEntity, @NotNull EntityMisc cap) {
        super.trigger(serverPlayer, (trigger) -> trigger.matches(serverPlayer, currentEntity, cap));
    }
    ...
    public class Instance extends AbstractCriterionTriggerInstance {
        ...
        public boolean matches(ServerPlayer serverPlayer, Entity currentEntity, @NotNull EntityMisc cap) {
            boolean entityMatch;
            if (this.isTargetTag) {
            ...
            int currentCount = 0;
            if (entityMatch) {
                currentCount = cap.mobsKilledTracker.getOrDefault(this.targetEntity, 0);
                currentCount += 1;
                cap.mobsKilledTracker.put(this.targetEntity, currentCount);
            }

            return entityMatch &&
                    (!beeArmorRequired || BeeArmor.getBeeThemedWearablesCount(serverPlayer) >= 4) &&
