...
public class HoneyCrystalShardRenderer extends EntityRenderer<HoneyCrystalShardEntity> {
    ...
    public void render(HoneyCrystalShardEntity honeyCrystalShardEntity, float float1, float float2, PoseStack poseStack, MultiBufferSource multiBufferSource, int int1) {
        poseStack.pushPose();
        poseStack.mulPose(Vector3f.YP.rotationDegrees(Mth.lerp(float2, honeyCrystalShardEntity.yRotO, honeyCrystalShardEntity.getYRot()) - 90.0F));
        poseStack.mulPose(Vector3f.ZP.rotationDegrees(Mth.lerp(float2, honeyCrystalShardEntity.xRotO, honeyCrystalShardEntity.getXRot()) + 90.0F));
