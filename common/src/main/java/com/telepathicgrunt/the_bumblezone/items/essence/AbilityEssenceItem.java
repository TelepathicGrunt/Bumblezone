...
public abstract class AbilityEssenceItem extends Item implements ItemExtension {
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int i, boolean bl) {
        if (stack.is(BzTags.ABILITY_ESSENCE_ITEMS) && entity instanceof ServerPlayer serverPlayer) {
            ...
            if (getForcedCooldown(stack)) {
                ...
                if (serverPlayer.getCooldowns().isOnCooldown(stack.getItem())) {
                    ItemCooldowns.CooldownInstance cooldownInstance = ((ItemCooldownsAccessor)serverPlayer.getCooldowns()).bumblezone$getCooldowns().get(stack.getItem());
                    int cooldownTime = ((ItemCooldownsAccessor)serverPlayer.getCooldowns()).bumblezone$getTickCount() - ((CooldownInstanceAccessor)cooldownInstance).bumblezone$getStartTime();
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> components, TooltipFlag tooltipFlag) {
        if (getIsLocked(stack)) {
            ...
            components.add(Component.translatable("item.the_bumblezone.essence_cooldown", GeneralUtils.formatTickDurationNoMilliseconds(getCooldownTickLength() - getCooldownTime(stack), 20.0f)).withStyle(ChatFormatting.DARK_RED));
            components.add(Component.empty());
            addDescriptionComponents(components);
        }
        else {
